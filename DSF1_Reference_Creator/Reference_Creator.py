...
FlatLine = -50                              # (dB) Amplitude line that spectrum will try to reach
ThresholdLevels = [-1, 1]                   # Threshold levels between which the spectrum power is accepted
...
SpectrumCheckDelay = 1                      # (sec) Interval between checks for an acquired OSA spectrum
...
def GenerateOffsetsFile():
    
    data = "OFFSETS\n\n"

    for spectralChannel in OffsetsTable:
        data += str(spectralChannel) + ":" + str(OffsetsTable[spectralChannel][0]) + "," + str(OffsetsTable[spectralChannel][1]) + "\n"

    file = open("reference_offsets_" + str(round(FlatLine, 2)) + ".txt", "w")
...
def main():
    ...
    # STEP 2: Connect to OSA
    print("\n-- STEP 2: CONNECTING OSA --")
    InitOSA()

    # STEP 3: Connect to XPOW
    print("\n-- STEP 3: CONNECTING XPOW --")
    InitXPOW()
    ...
    #STEP 4: Discard spectrums
    print("\n-- DISCARDING " + str(DiscardedSpectrumNum) + " SPECTRUMS --")
    ...
    for iteration in range(Iterations):
        ...
        for i in range(len(SpectralChannels)):
            ...
            diffPowers.append(GetSpectralPower(SpectralChannels[i], [spectrumScanData[0], FlatLine - spectrumScanData[1]]))

        if (GenerateGraphs == True):
            targetData = np.empty(shape = len(spectrumData[0]), dtype = float)

            for i in range(len(spectrumData[0])):
                targetData[i] = FlatLine

            smoothData = GaussianSmoothData(spectrumData, [1400, 1800])
            ...
            plt.xlim(1400, 1800)
            plt.ylim(-100, 0)
            plt.plot(spectrumData[0], targetData, label = "Target Line", color = "#ff7f0e")
            ...
            #plt.plot(smoothData[0], smoothData[1], label = "Smooth Spectrum Data", color = "#1f77b4")
            plt.legend()
            #plt.show()
        ...
        for i in range(len(spectralPowers)):
            targetDiffPower = 0
            print("Spectral channel " + str(SpectralChannels[i]) + " is at " + str(round(spectralPowers[i], 2)) + " dB. Difference is " + str(round(diffPowers[i], 2)) + " dB, target difference is " + str(round(targetDiffPower, 2)) + " dB.")
            ...
            if (diffPowers[i] < targetDiffPower + ThresholdLevels[0]) or (diffPowers[i] > targetDiffPower + ThresholdLevels[1]):
                ...
                supposedPower = GetLookupAmplitude(CheckXPOWChannels[i], SpectralChannels[i], CurrVoltages[SpectralChannels[i]])
                possibleTargetVoltages = GetLookupVoltage(CheckXPOWChannels[i], SpectralChannels[i], supposedPower + diffPowers[i])
                ...
                print("Difference power is " + str(round(diffPowers[i], 2)) + " dB. Target difference power is " + str(round(targetDiffPower, 2)) + " dB.")
                print("Current appplied voltage of spectral channel " + str(SpectralChannels[i]) + " is " + str(round(CurrVoltages[SpectralChannels[i]], 2)) + "V.")
                ...
                print("Power difference of " + str(round(diffPowers[i], 2)) + " dB. Adjusting by " + str(round(diffVoltage, 2)) + "V, new applied voltage is " + str(round(CurrVoltages[SpectralChannels[i]], 2)) + "V.")

                OffsetsTable[SpectralChannels[i]] = [supposedPower, CurrVoltages[SpectralChannels[i]]]
    ...
    # STEP 6: Save reference spectrum
    print("\n-- STEP 6: SAVING REFERENCE --")

    GetOSASpectrum()

    FileInterface.WriteSpectrum(CurrSpectrum, "reference_spectrum_" + str(round(FlatLine, 2)) + ".txt", 1)
    FileInterface.WriteSpectrum(CurrSpectrum, "reference_spectrum_" + str(round(FlatLine, 2)) + ".spf2", 2)

    # STEP 7: Save reference offsets
    print("\n-- STEP 7: SAVING OFFSETS --")

    GenerateOffsetsFile()

    # STEP 8: Close devices
    print("\n-- STEP 8: CLOSING DEVICES --")
